function varargout = TAgui_cmd_historywindow(varargin)
% TAGUI_CMD_HISTORYWINDOW Window displaying the directories used
...
try
    ...
    TAmsg('Cmd history window opened.','info');
        ...
        switch lower(action)
            case 'evaluate'
                ...
                for commands=1:length(values)
                    [status,warning] = TAguiCommand(char(history(...
                        values(commands))));
                    if status
...
function closeGUI(~,~)
    try
        delete(hMainFigure);
        TAmsg('Cmd history window closed.','info');
    catch exception
        try
            msgStr = ['An exception occurred in ' ...
                exception.stack(1).name  '.'];
