function varargout = TAgui_setdirectorieswindow(varargin)
% TAGUI_SETDIRECTORIESWINDOW Window displaying the directories used
...
hMainFigure = figure('Tag',mfilename,...
    'Visible','off',...
    'Name','TA GUI : Show/Set Directories Window',...
    'Units','Pixels',...
...
uicontrol('Tag','heading_text',...
    ...
    'String','Show/Set directories used internally in the TA GUI'...
    );
...
try
    ...
    TAmsg('Show/Set directories window opened.','info');
...
function pushbutton_Callback(~,~,action)
    try
        ...
        admain = getappdata(TAguiGetWindowHandle());
    ...
    try
        ...
        switch lower(action)
            case {'loaddir','savedir','savefigdir','exportdir','snapshotdir'}
                cmdDir(TAguiGetWindowHandle(),{lower(action),directory});
            otherwise
                disp(['Action ' action ' not understood.'])
                return;
        end
...
function closeGUI(~,~)
    try
        clear('jObject');
        delete(hMainFigure);
        TAmsg('Show/Set directories window closed.','info');
    catch exception
        try
            msgStr = ['An exception occurred in ' ...
                exception.stack(1).name  '.'];
...
function updateWindow()
    try
        % Get appdata from main TA GUI handle
