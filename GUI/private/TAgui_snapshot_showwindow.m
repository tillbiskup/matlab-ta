function varargout = TAgui_snapshot_showwindow(varargin)
% TAGUI_SNAPSHOT_SHOWWINDOW Window displaying snapshots from a given
...
function closeGUI(~,~)
    try
        delete(hMainFigure);
        TAmsg('Snapshot window closed.','info');
    catch exception
        try
            msgStr = ['An exception occurred in ' ...
                exception.stack(1).name  '.'];
