% TAEXPORT1D Export 1D cross sections of 2D dataset
%
% Usage
%   TAexport1D(data,fileName,parameters);
%   status = TAexport1D(data,fileName,parameters);
%
% data       - struct
%              data to plot
        ...
        case 'x'
            ...
            switch parameters.display.MFEdisplay
                ...
                case 'sum(MFoff,MFon)'
                    crosssection = ...
                        (data.dataMFon(parameters.crosssection.position,:)'+...
                        data.data(parameters.crosssection.position,:)')/2;
            end
            ...
            if parameters.axis.include && parameters.stdev.include && ...
                ...
                switch parameters.display.MFEdisplay
                    ...
                    case 'sum(MFoff,MFon)'
                        for l=1:length(crosssection)
                            fprintf(fid,'%20.12f %20.12f %20.12f\n',axis(l),...
                                crosssection(l),data.avg.stdev.SumMF(l));
                        end
                end
