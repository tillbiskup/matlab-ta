...
try
    ...
    for k=1:length(matchingWavelengths)
        if ~isnumeric(getCascadedField(parameters,matchingWavelengths{k,1}))
            parts = regexp(...
                getCascadedField(parameters,matchingWavelengths{k,1}),...
                ' ','split','once');
            dataStructure = setCascadedField(dataStructure,...
                matchingWavelengths{k,2},str2double(parts{1}));
            if length(parts) > 1
                dataStructure.parameters.probe.wavelength.unit = parts{2};
            end
        else
            dataStructure = setCascadedField(dataStructure,...
                    matchingWavelengths{k,2},...
                    getCascadedField(parameters,matchingWavelengths{k,1}));
        end
    end
